 ...
 * @return  void
 */
void HPMA115S0::SendCmd(const unsigned char * cmdBuf, unsigned int cmdSize) {
  //Clear RX
...
int HPMA115S0::ReadCmdResp(unsigned char * dataBuf, unsigned int dataBufSize, unsigned int cmdType) {
  ...
  if (_serial.readStringUntil(HPM_CMD_RESP_HEAD)) {
    ...
    if (respBuf[HPM_LEN_IDX] && ((respBuf[HPM_LEN_IDX] + (unsigned int)1) <=  sizeof(respBuf) - (unsigned int)2) && (respBuf[HPM_LEN_IDX] - (unsigned int)1) <= dataBufSize ) {
      if (_serial.readBytes(&respBuf[HPM_CMD_IDX], respBuf[HPM_LEN_IDX] + (unsigned int)1) == (respBuf[HPM_LEN_IDX] + (unsigned int)1)) { //read respBuf[HPM_LEN_IDX] num of bytes + calChecksum byte
        if (respBuf[HPM_CMD_IDX] == cmdType) { //check if CMD type matches

          //Calculate and validate checksum
          for (respIdx = 0; respIdx < ((unsigned int)2 + respBuf[HPM_LEN_IDX]); respIdx++) {
...
boolean HPMA115S0::ReadParticleMeasurement(unsigned int * pm2_5, unsigned int * pm10) {
  const unsigned char cmdBuf[] = {0x68, 0x01, 0x04, 0x93};
...
void HPMA115S0::StartParticleMeasurement() {
  const unsigned char cmd[] = {0x68, 0x01, 0x01, 0x96};
  SendCmd(cmd, 4);
}
...
void HPMA115S0::StopParticleMeasurement() {
  const unsigned char cmd[] = {0x68, 0x01, 0x02, 0x95};
  SendCmd(cmd, 4);
}
...
void HPMA115S0::EnableAutoSend() {
  const unsigned char cmd[] = {0x68, 0x01, 0x40, 0x57};
  SendCmd(cmd, 4);
}
...
void HPMA115S0::DisableAutoSend() {
  const unsigned char cmd[] = {0x68, 0x01, 0x20, 0x77};
  SendCmd(cmd, 4);
}
